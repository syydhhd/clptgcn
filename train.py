...
class Instructor:
    ...
    def get_bert_optimizer(self, model):
        ...
        if self.opt.diff_lr:
            ...
            param_group = [
                {'params': [p for n, p in params if not any(nd in n for nd in no_main)], 'weight_decay': self.opt.weight_decay,
                 'lr': self.opt.bert_lr},
                ...
                {'params': [p for n, p in params if
                            any(nd in n for nd in other) and not any(nd in n for nd in no_decay)], 'weight_decay': self.opt.weight_decay,
                 'lr': self.opt.learning_rate},
            ]
    ...
    def _train(self, criterion, optimizer, max_test_acc_overall, contrastiveLoss_sentiment):
        # path = 'state_dict/dualgcnbert_laptop_acc_0.8212_f1_0.7882'
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_dataloader):
                ...
                loss = criterion(logits, targets) + loss_cl * self.opt.l1 + criterion(logits_pos, targets)*self.opt.l2 +\
                    loss_dl*self.opt.l6 + loss_kl*self.opt.l3 + criterion(scores, label_id)*self.opt.l4 + loss_scl * self.opt.l5
                loss.backward()
                optimizer.step()
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    if f1 > max_f1:
                        max_f1 = f1
                    logger.info('loss: {:.4f}, acc: {:.4f}, test_acc: {:.4f}, f1: {:.4f}, loss_cl: {:.4f}, loss_kl: {:.4f}'.format(loss.item(), train_acc, test_acc, f1, loss_cl, loss_kl))
            time_end = time.time()
    ...
    def _evaluate(self, show_results=False):
        ...
        with torch.no_grad():
            for batch, sample_batched in enumerate(self.test_dataloader):
                ...
                pooled_out_all = torch.cat((pooled_out_all, pooled_out), dim=0) if pooled_out_all is not None else pooled_out
                # target_weight_all = torch.cat((target_weight_all, target_weight), dim=0) if target_weight_all is not None else target_weight
                # input_ids_all = torch.cat((input_ids_all, inputs[0]),
...
def main():
    ...
    dataset_files = {
        'restaurant': {
            'train': 'D:\codehe\clpt-GCN\dataset/Restaurants_allennlp/train.json',
            'test': 'D:\codehe\clpt-GCN\dataset/Restaurants_allennlp/test.json',
        },
        'laptop': {
            'train': 'D:\codehe\clpt-GCN\dataset/Laptops_allennlp/train.json',
            'test': 'D:\codehe\clpt-GCN\dataset/Laptops_allennlp/test.json'
        },
        'twitter': {
            'train': 'D:\codehe\clpt-GCN\dataset/Tweets_allennlp/train.json',
            'test': 'D:\codehe\clpt-GCN\dataset/Tweets_allennlp/test.json',
        },
        'mams': {
            'train': 'D:\codehe\clpt-GCN\dataset/MAMS/train_con_new.json',
            'test': 'D:\codehe\clpt-GCN\dataset/dataset/MAMS/test_con_new.json',
        }
    }

    input_cols = {
        'clpt': ['text_bert_indices_mask', 'text_bert_indices_pos', 'text_bert_indices_neg_1', 'text_bert_indices_neg_2', 'bert_segments_ids_aul', 'attention_mask_aul','adj_matrix', 'src_mask', 'aspect_mask',
                  'label_id', 'loc_mask']
    }
    ...
    # * bert
    parser.add_argument('--pretrained_bert_name', default='E:\codehe\clpt-GCN\plm/bert-base', type=str)
    ...
    parser.add_argument('--l3', default=0.0, type=float)
    parser.add_argument('--l4', default=0.0, type=float)
    parser.add_argument('--l5', default=0.0, type=float)
